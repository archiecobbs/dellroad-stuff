...
public class PipedStreamsTest extends TestSupport {
    ...
    public void testPipedStreams() throws Exception {
        ...
        final ByteArrayOutputStream readerBuffer = new ByteArrayOutputStream();
            ...
            try (OutputStream out = ps.getOutputStream()) {
                for (int i = 0; i < 1000; i++) {
                    if (this.random.nextInt(10) == 7) {
                        ...
                        writerBuffer.write(data, off, len);
        ...
        Assert.assertEquals(readerDigest.digest(), writerDigest.digest(),
          "reader digest != writer digest"
          + "\nReader = " + ByteArrayEncoder.encode(readerBuffer.toByteArray())
          + "\nWriter = " + ByteArrayEncoder.encode(writerBuffer.toByteArray()));
    }
